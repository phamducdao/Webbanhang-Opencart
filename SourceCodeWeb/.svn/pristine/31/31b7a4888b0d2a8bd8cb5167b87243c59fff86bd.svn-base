...
class ModelNewsCategory extends Model {
		...
		if (isset($data['category_seo_url'])) {
			foreach ($data['category_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (!empty($keyword)) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET store_id = '" . (int)$store_id . "', language_id = '" . (int)$language_id . "', query = 'category_news_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($keyword) . "'");
					}
				}
			}
		}
	...
	public function editCategory($category_id, $data) {
		...
		if ($query->rows) {
			foreach ($query->rows as $category_news_path) {
				// Delete the path below the current one
				$this->db->query("DELETE FROM `" . DB_PREFIX . "category_news_path` WHERE category_id = '" . (int)$category_news_path['category_id'] . "' AND level < '" . (int)$category_news_path['level'] . "'");
				...
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_news_path` WHERE category_id = '" . (int)$category_news_path['category_id'] . "' ORDER BY level ASC");
				...
				foreach ($path as $path_id) {
					$this->db->query("REPLACE INTO `" . DB_PREFIX . "category_news_path` SET category_id = '" . (int)$category_news_path['category_id'] . "', `path_id` = '" . (int)$path_id . "', level = '" . (int)$level . "'");
	...
	public function getCategoryDescriptions($category_id) {
		...
		return $category_news_description_data;
	}

	public function getCategoryPath($category_id) {
		$query = $this->db->query("SELECT category_id, path_id, level FROM " . DB_PREFIX . "category_news_path WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategorySeoUrls($category_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE query = 'category_news_id=" . (int)$category_id . "'");
